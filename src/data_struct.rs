...
pub struct Node {
    node_id: u32,
    timestamp: u64,
    // The edges of the graph are stored directly in the node in the form fof vecotr of node_id
    // Since the graph is fairly simple and restricted in size it simplify its construction and exploration (at the cost of performance)
    parents: Vec<u32>,
    children: Vec<u32>,
}
...
impl Graph {
    ...
    pub fn parse(&mut self, filename: String) {
        ...
        let _res = match _res {
            Ok(usize) => usize, 
            Err(error) => {
                panic!("Error reading first line: {:?}", error)
            },
        };
        ...
        let mut tmp_vec: Vec<Pair> = Vec::new();
        let mut index = 1;
        ...
        for p in tmp_vec {
            ...
            self.nodes[(p.p0) as usize].add_parent(p.p1);
            self.nodes[(p.p0) as usize].add_parent(p.p2);
            self.nodes[(p.p1) as usize].add_child(p.p0);
            self.nodes[(p.p2) as usize].add_child(p.p0);
        }
    ...
    pub fn show(&self) {
        ...
        for node in &self.nodes {
            ...
            let mut depth = Graph::get_depth(&mut self.nodes.clone(), node.node_id);
            println!(
